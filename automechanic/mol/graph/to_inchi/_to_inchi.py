...
def with_numbering(cgr, is_chi=False, atm_xyz_dct=None):
    ...
    """
    atm_keys = atom_keys(cgr)
    bnd_keys = bond_keys(cgr)
    atm_syms = _values_by_key(atom_symbols(cgr), atm_keys)
    atm_bnd_vlcs = _values_by_key(atom_bond_valences(cgr), atm_keys)
    atm_rad_vlcs = _values_by_key(atom_radical_valences(cgr), atm_keys)
    atm_xyzs = (None if atm_xyz_dct is None else
                _values_by_key(atm_xyz_dct, atm_keys))
    bnd_ords = _values_by_key(bond_orders(cgr), bnd_keys)
    ...
    assert set(ich_srt_bbn_keys) == set(backbone_keys(cgr))
