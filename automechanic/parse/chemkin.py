...
def reaction_data_reactant_names(rxn_dstr):
    """ get the reactants from a reaction data string
    """
    rxn = reaction_data_reaction_name(rxn_dstr)
    rct_str, _ = find_split(CHEMKIN_ARROW, rxn)
    rcts = _split_reagent_string(rct_str)
    return rcts


def reaction_data_product_names(rxn_dstr):
    """ get the products from a reaction data string
    """
    rxn = reaction_data_reaction_name(rxn_dstr)
    _, prd_str = find_split(CHEMKIN_ARROW, rxn)
...
def thermo_data(mech_str):
    ...
    thm_dat_lst = tuple(zip(
        ...
        map(thermo_data_low_t_coefficients, thm_dstr_lst),
        map(thermo_data_high_t_coefficients, thm_dstr_lst),
        map(thermo_data_t_common, thm_dstr_lst)))
    return thm_dat_lst
...
def thermo_data_t_common(thm_dstr):
    """ get the common temperature from a thermo data string
    """
    headline = find_split_lines(thm_dstr)[0]
    pattern = (SPACES + UNSIGNED_FLOAT +
               SPACES + UNSIGNED_FLOAT +
               SPACES + capturing(UNSIGNED_FLOAT))
    capture = find_single_capture(pattern, headline)
    assert capture
    tmp_com = float(capture)
    return tmp_com


def thermo_data_low_t_coefficients(thm_dstr):
...
def thermo_data_high_t_coefficients(thm_dstr):
    """ get the low temperature thermo coefficients
    """
    capture_lst = find_captures(EXPONENTIAL_FLOAT, thm_dstr)
    assert len(capture_lst) in (14, 15)
